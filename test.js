...
function showResult(questionContainerId, resultElementId) {
    ...
    // Определяем породу с наибольшим количеством баллов
    let maxScore = 0;
    let winningBreed = "";

    for (let breed in scores) {
    ...
    resultContainer.innerHTML = `<p>Поздравляю, ты отличный владелец собаки породы ${winningBreed} !</p>`;

    const resultImage = document.createElement('img');
    resultImage.src = breedImages[winningBreed];
    resultImage.alt = winningBreed;
    resultImage.style.width = "300px"; // Устанавливаем ширину изображения
    resultImage.style.borderRadius = "5px"; // Добавляем закругление углов
    
    resultContainer.appendChild(resultImage);
    /* метод appendChild используется для добавления указанного узла (resultImage) 
    в качестве последнего дочернего элемента к элементу resultContainer. 
    Метод изменяет структуру DOM (Document Object Model) на странице, добавляя изображение в указанный контейнер.
    */
    
    resultContainer.style.display = "block";  /* делаем элемент видимым, чтобы пользователь мог увидеть результаты теста */
